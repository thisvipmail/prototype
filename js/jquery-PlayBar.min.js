...
(function($) {
    ...
    var beginTime = "";
    jQuery.playBar = {
        addBar: function(DOM, allTime,beginTime_) {
            CleanAll();
            alltime = allTime;
            DOM.empty();
            DOM.append("<div class='BarControl'><div class='beginTime'>2017-01-04 15:42</div><div class='BarBeginTime'>00:00</div></div>");
    ...
    $(document).mouseup(function() {
        if (BarMove) {
            ...
            if (isAction) {
                var ret = ButtonStartTimePlaybackByHWND_timeChange(new Date(new Date(beginTime).getTime()+CurrTime))
                if(ret == 0){
                    OpenBar();
                }
            }
    ...
    function timechange() {
        ...
        var date = new Date(new Date(beginTime).getTime()+CurrTime);
        $('.beginTime').html(date.getFullYear()+"/"+(date.getMonth()+1)+"/"+date.getDate()+" " + date.getHours() + ":"+date.getMinutes()+":"+date.getSeconds());
        //$('.beginTime').html(data.toLocaleString());
    ...
    function changeBar() {
        ...
        if (thewidth < rwidth && CurrTime < alltime) {
            ...
            var date = new Date(new Date(beginTime).getTime()+CurrTime);
            $('.beginTime').html(date.getFullYear()+"/"+(date.getMonth()+1)+"/"+date.getDate()+" " + date.getHours() + ":"+date.getMinutes()+":"+date.getSeconds());
            //$('.beginTime').html(date.toLocaleString());
        } else {
            thewidth = rwidth;
            StopBar()
        }
